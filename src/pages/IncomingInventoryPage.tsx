...
export default function IncomingInventoryPage() {
    ...
    if (searchTerm) {
      const filtered = movements.filter((movement) => {
        ...
        return (
          ...
          (movement.warehouseName && typeof movement.warehouseName === 'string' && movement.warehouseName.toLowerCase().includes(searchLower)) ||
          (movement.supplierName && typeof movement.supplierName === 'string' && movement.supplierName.toLowerCase().includes(searchLower)) ||
  ...
  const getDisplayName = (name: any): string => {
    ...
    if (name && typeof name === 'object' && 'name' in name) return name.name;
    return '-';
  };
            ...
            {loading ? (
              ...
              filteredMovements.map((movement) => (
                <TableRow key={movement.id} className={movement.status === "cancelled" ? "bg-muted/50" : ""}>
                  ...
                  <TableCell>{getDisplayName(movement.warehouseName)}</TableCell>
                  <TableCell>{getDisplayName(movement.supplierName)}</TableCell>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div>{format(parseISO(selectedMovement.date), "PPP", { locale: es })}</div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div>{getDisplayName(selectedMovement.warehouseName)}</div>
                </div>

                {/* Proveedor */}
                <div className="space-y-1">
                  <h4 className="text-sm font-medium text-muted-foreground">Proveedor</h4>
                  <div>{getDisplayName(selectedMovement.supplierName)}</div>
                </div>

                {/* Registrado por */}
                <div className="space-y-1">
                  <h4 className="text-sm font-medium text-muted-foreground">Registrado por</h4>
                  <div>{selectedMovement.createdBy || "-"}</div>
                </div>
                ...
                {selectedMovement.status === "cancelled" && (
                  <>
                    <div className="space-y-1">
                      ...
                      <div>{selectedMovement.updatedBy || "-"}</div>
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-sm font-medium text-muted-foreground">Motivo de anulación</h4>
                      <div>{selectedMovement.cancellationReason || "-"}</div>
                    </div>
                  </>
                )}
              </div>
                  ...
                  <TableBody>
                    {selectedMovement.lines.map((line) => {
                      ...
                      return (
                        <TableRow
                          key={line.id}
                          className={cn(selectedMovement.status === "cancelled" && "text-muted-foreground")}
                        >
                          ...
                          <TableCell className="text-right">
                            {line.unitCost ? `$${lineTotal.toFixed(2)}` : "-"}
                          </TableCell>
                        </TableRow>
                      );
                    })}
                    ...
                    <TableRow>
                      ...
                      <TableCell className="text-right font-medium">
                        {selectedMovement.totalItems || 0}
                      </TableCell>
                      <TableCell></TableCell>
                      <TableCell className="text-right font-medium">
                        ${selectedMovement.lines
                          .reduce((sum, line) => sum + (line.unitCost ? line.quantity * line.unitCost : 0), 0)
                          .toFixed(2)}
                      </TableCell>
                    </TableRow>
                  </TableBody>
